...
/usr/local/etc/srsr.conf、又は、$HOME/.srsr.conf
  SRSR_OUTDIR=      保存ファイルを出力するデフォルトの場所を設定。`$HOME/Downloads'等
  ...
  SRR_OMCH=         srr.sh、srs.shでチャンネル名を指定しなかった場合に設定されるチャンネル
  SRR_OMDURATION=   srr.sh、srs.shで時間指定しなかった場合に設定される分数
  SRSR_BINDIR=      authkey.sh、getwtt.sh、mkagtt.sh、mskinfo.shを置く場所
...
/SRSR_VARTMPDIR/tt_AG_over60
  ...
  * $((`19'の部分が19時を、`Sat=120'の部分が土曜=番組の時間120分を指し、
    $((`20'と`Sat=0'の部分で、土曜20時台に番組が無いようにみせています。

  29分番組がある木曜17時台は合計59分になりますが、この場合も120分の場合を応用して、
  [ "$th" = $((17 - 6)) ] && Thu=59
